...
CREATE POLICY "Users can view today's templates for their plan"
ON public.task_templates
FOR SELECT
USING (
  is_active = true
...
CREATE POLICY "Users can view their user task statuses"
ON public.user_task_status
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their user task statuses"
ON public.user_task_status
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their user task statuses"
ON public.user_task_status
FOR UPDATE
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);
